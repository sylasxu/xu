...
- **用户可见内容配置规则**:
  - 所有面向用户的业务文案、标题、副标题、placeholder、CTA、empty state、toast、sheet 文案、选项 label、引导语，都必须支持“入库 + 后端获取”，禁止只存在于前端代码
  - 优先使用领域数据或后端配置下发；欢迎页、对话壳层、消息中心、反馈/举报、分享文案等通用 UI 文案，统一走 `ai_configs` 或明确的领域接口返回
  - 前端允许保留兜底值，但兜底只能是通用/中性/system fallback，不能承载正式产品文案，更不能和后端配置各写一套
  - 新增界面文案时，必须同时补齐：配置键或领域字段、API/Schema、seed/默认值、前端消费；禁止先把文案写死在组件里“以后再抽”
  - `aria-label`、开发日志、纯技术错误、平台原生固定文案可留在前端；除此之外的可见业务内容默认都视为需要后端治理
- **React Bits 使用边界**:
  - 只允许作为视觉包装层使用，例如 welcome 背景、输入框 wrapper、轻量装饰层
  - 禁止用 React Bits 替换聊天主列表、消息协议、SSE 承接逻辑
- **视觉方向**:
  - 首页 `H0` 空态可以有克制的暗色氛围与包装
  - 首页所有动态/发光/轨道类特效都必须是最底层 `bg` 氛围层，只能 `pointer-events-none` + `aria-hidden`，不得进入文案、任务卡、CTA 的内容层；内容层必须有足够遮罩或实底，避免特效穿过文字
  - 首页 header 必须是透明浮层，不允许用实色背景块切断 bg 氛围层；导航按钮自身可以有轻边界或轻底色，但 header 容器不能遮挡背景
  - 进入任务主舞台后必须收敛到纯黑/近纯黑内容舞台，让 GenUI cards 成为主角
...
- **用户可见内容配置规则**:
  - 与 H5 一致：所有用户可见的业务内容必须支持数据库存储和后端下发，禁止把 welcome、空状态、弹窗文案、选项、提示语、反馈/举报文案长期写死在小程序组件里
  - 小程序页面/组件只能消费接口返回的内容配置或领域数据；本地常量仅作为通用兜底，不得成为正式内容真源
  - 若接口暂未返回所需文案，正确做法是补 `@xu/db` / API / Orval 协议，不是继续新增本地字符串
