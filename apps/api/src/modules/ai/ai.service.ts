 ...
 *
 * v6.3: 移除 buildDeterministicActionVoice，LLM 失败时统一返回简洁兜底文案，不再维护手写 fallback。
 */
const ACTION_LABEL_MAP: Record<string, string> = {
  ...
  view_activity: '查看活动',
  share_activity: '分享活动',
  opt_in_partner_pool: '帮我留意',
  cancel_join: '取消报名',
  record_activity_feedback: '给个反馈',
  confirm_match: '确认匹配',
  cancel_match: '取消匹配',
  submit_partner_intent_form: '提交意向',
};
...
- ${hasError ? '温和告知失败，给一个轻松的替代建议' : '轻松确认成功，暗示下一步可以做什么'}`;
  ...
  try {
    ...
    // 给 generateObject 加 5s 超时保护，防止模型/网络异常时无限 hang
    ...
    const nextActions = followupActions
      .map((action, index) => ({
        label: (object.labels[index] || '').trim() || resolveActionLabel(action.action),
    ...
    return {
      text: object.text.trim() || systemMessage || '稍等，我帮你再看看～',
      nextActions,
    };
  } catch (error) {
    logger.error('action_response_failed', { actionType, error: String(error) });
    return {
      text: systemMessage || '稍等，我帮你再看看～',
      nextActions: followupActions.map((a) => ({ label: resolveActionLabel(a.action), ...a })),
    };
  }
}
