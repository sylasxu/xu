...
const TOPIC_SUGGESTION_PROMPT = `你不是来写正文的，你是 xu 内容运营的“主题起手助手”。
...
4. 要贴近 xu 真实场景：重庆、同城、周末活动、约饭、羽毛球、桌游、散步、喝茶聊天、找搭子、组局、谁组我就去
...
async function generateNoteObject(
  ...
  try {
    const result = await generateObject({
      ...
      schemaDescription: 'xu Admin 内容生成结果，必须返回 title、body、hashtags、coverText、coverImageHint 的 JSON 对象',
...
async function generateTopicSuggestionObject(
  ...
  try {
    const result = await generateObject({
      ...
      schemaDescription: 'xu Admin 内容主题建议结果，必须返回 items 数组的 JSON 对象',
