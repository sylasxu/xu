...
> **版本**：v5.4 (Editorial Dark Welcome + Structured Action Flow + Action-Gated Auth)
> **更新日期**：2026-04-11
> **App 名称**：xu
> **产品定位**：碎片化社交助理（对外） / 持续任务运行时（对内实现）
> **核心 Slogan**：今天有什么想玩的？
> **AI 角色名**：xu
...
- 对外，xu 是帮助用户把碎片化社交需求整理成可推进结果的助理
- 对内，系统通过持续任务运行时、结构化动作和会话上下文持续推进同一件事
- 产品要复刻的不是抽象 AI 能力，而是“听懂群里一句话、知道这是什么局、知道下一步怎么接、能把事续到成局或收尾”的组织能力

| 维度 | 传统工具 (SaaS) | xu（当前实现） |
|------|----------------|--------------------------|
| **交互模式** | 用户填表单、点按钮 | 用户说话，AI 理解并执行 |
| **界面生成** | 静态 UI，所有人看到一样的 | 根据意图返回不同的结果卡片和交互块 |
| **服务对象** | 只服务"群主"（发起者） | 服务每一个人（发起者 + 参与者） |
| **用户关系** | 用完即走 | 尽量减少重复输入，保留上下文 |
| **核心价值** | 效率工具 | 更容易参加、组织和承接一场局 |
...
| **S2 表达** | “我想找局/找搭子” | 统一对话入口接收 `text/action` | 输入回显 + 思考气泡 | 成功进入意图识别或结构化动作链路 |
...
> **判断标准**：不是“功能是否存在”，而是“群里一句话冒出来时，xu能不能像一个真正会张罗的群主一样，把这件事顺着接住并办下去”。

| 场景 | 用户真实表达 | 当前判断 | 已成立能力 | 仍未到完美的点 |
|------|-------------|----------|-----------|---------------|
| **找局** | “附近有什么局”“观音桥有没有火锅局”“这个活动我能不能直接报” | **已支持，但未到完美** | 首页 Chat 可进入 explore；`join_activity` 已形成从探索、报名、auth gate、讨论区到活动后 follow-up 的连续任务；报名被打断后可恢复同一条动作；报名成功后统一进入讨论区 | 前台承接感还不够强，用户不一定明显感知到“还是同一个xu在继续办这件事” |
...
| **活动后续上** | “这场怎么样”“帮我复盘一下”“下次还想约”“谁真正到场了” | **闭环已打通，但体验还没完全长出来** | `confirm-fulfillment`、review、rebook、memory 写回已成真链路；带 `activityId + followUpMode` 的入口会挂回原活动；`join_activity` 可推进到 `post_activity -> done` | 逻辑已完整，但用户心智还没完全建立成“xu最擅长接后半段”的招牌体验 |
...
- 当前产品主引擎已经明确为“找搭子 / 组局”Agent；内容工作台不是独立业务线，而是把这些真实需求翻成外部分发内容的放大器

#### 1.2.0.4 当前阶段判断（团队内部同步口径）

> **当前阶段**：xu已经走出“方向验证期”，进入“体验收口期”。

这意味着：

- 不需要推翻产品原点或交互骨架
- 主矛盾不再是“做什么”，而是“把最重要的真实场景做顺、做稳、做得像人”
- 后续判断标准不是“有没有再加功能”，而是“xu是否更像一个真的会张罗组局的群主”

**已经成立**：

- 产品原点成立：不是抽象 AI 概念，而是来自真实群聊里的找局、组局、找搭子、活动后续上
- 对话入口成立：用户可以像在群里说话一样开口，不需要先学复杂模块
...
- 首页和消息中心的连续承接感还不够强，用户未必始终感知到“还是同一个xu在继续办”
...
xu现在已经是一个“方向对、主链路成立、但仍需继续收口”的产品；接下来最重要的不是怀疑要不要重做，而是把最关键的真实场景打磨到更像一个会张罗的群主。

#### 1.2.1 [Flow] 找搭子 (Partner Matching) - 核心痛点攻坚

> **痛点 vs 解法**
> | 旧世界（微信群/朋友圈） | 新世界（xu） | 业务价值 |
> |-------------------------|-------------------------|----------|
...
**跨端收口要求（v5.5）**：
- 这条链路的目标是“降低确认参加的动作负担”，不是在产品里复刻“私信群主”
- 用户一旦进入xu，默认已经脱离微信群公开发言压力，因此报名动作不应再附带自我介绍、社交表演或额外说明
- 活动详情、半屏详情、地图探索、AI 推荐卡触发的 `join_activity` 必须共享同一套结果语义：`joined | already_joined | waitlisted | closed`
- `joined / already_joined` 统一承接到讨论区，`waitlisted / closed` 统一停留在详情态并给明确反馈
...
但好消息是：我们现在这套“对话入口 + 动态界面块”架构，刚好站在了他们的反面。

#### 1.3.2 本质模型的差异：商城 vs. 秘书

| 维度 | 粗门 (The Mall) | xu (The Agent) |
|------|-----------------|------------------|
...
| 粗门现状 | xu解法 |
|----------|----------|
| 进去全是卖票的（9.9元/99元），全是专业的宣传海报 | **Editorial Dark**：界面克制、没有广告、没有推销 |
...
| 粗门现状 | xu解法 |
|----------|----------|
| 我就想约 3 个朋友吃个火锅，还要去粗门建个活动、审核、发布？疯了吗？ | **一句话成局**：这就是对话入口最适合的场景 |
| 对于"高频、低客单价、熟人/半熟人"的局，粗门是大炮打蚊子 | xu是精准手术刀 |
...
| 朋友聚餐 (火锅、烧烤、家宴) | 🟢 **xu 胜** | 粗门做不了。太重了，用户不会为了吃饭去发个商业活动 |
| 临时起意 (今晚去哪喝一杯？谁有空？) | 🟢 **xu 胜** | 唯快不破。对话入口更适合临时起意和快速摇人 |
| 半熟人拼车/拼单 (剧本杀缺1、拼车去机场) | 🟢 **xu 胜** | 信任感。基于群聊的信任背书，加上 Widget 的清晰展示 |
| 不知道去哪 (探索周边) | 🟢 **xu 胜** | AI 意图识别。粗门要你自己在列表里搜，xu是直接告诉你"附近有啥" |

**差异化总结**：
- **粗门垄断了头部 20% 的"严肃/商业活动"**
- **xu要吃掉长尾 80% 的"生活/社交活动"**
...
> **"粗门是给俱乐部卖票的电商平台；xu是你的 AI 活动助理。"**
>
> **"你想去参加百人徒步，去粗门。你想约几个朋友吃火锅、打麻将、拼个车，用 xu。"**

#### 1.3.8 名字的意义

现在回看"xu"这个名字：

> **我们守住的是生活里最真实的那些"xu"，而那里才是 99% 的流量所在。**

---

### 1.4 活动讨论区 (Activity Discussion)

> 基于 WebSocket 的实时通讯功能，为活动参与者提供讨论空间。
...
### 1.5 ~~AI 海报生成~~ → H5 邀请函替代

> Puppeteer 截图海报方案已被 H5 邀请函链接（`/invite/:id`）完全替代。
...
> 当前内容生成主入口统一使用 `POST /content/generate` 与 `POST /content/topic-suggestions`。生成结果除正文外，还应包含可直接用于首图的短文案与配图提示词；在小红书模式下，还应补充评论区引导句、私聊承接话术和转微信前参考句，方便 Solo Founder 按“发帖 -> 评论承接 -> 私聊筛选”完整执行。生成文案必须避免在公开内容里直接出现站外引流表达（如加群、私信、扣 1、主页联系等）。
>
> 当前执行定位：`找搭子 / 组局 Agent` 是主引擎，负责承接真实需求、推进成局与分享；内容工作台是它的外部分发器，用来把高频需求翻成可批量投放的小红书 / 抖音 / 微信内容。
...
> **当前生效定位（v5.5）**：基于 Next.js 的 H5 Web 应用，主要承担跨平台活动邀请函展示、活动信息预览，以及把用户引导回微信内完成参与。
...
|---------|------------------|-------------|
| 抖音直播间 | ❌ 无法分享小程序链接 | ✅ 分享 `xu.example/invite/xxx` |
...
- **核心目标**：让用户在微信外先看清活动信息，再进入微信内参与
- **当前链路**：外部曝光 → H5 邀请函 → 微信内转化

#### 1.7.2 `/invite/:id` 活动邀请函

**URL 格式**：`https://xu.example/invite/{activityId}`
...
>
> **理由**：xu 的流量模型是"外部曝光 → 微信内转化"。H5 的核心 KPI 是制造 FOMO（主题背景 + 社交氛围 + 报名人数），激发用户去微信内参与。加 H5 登录态会显著增加复杂度（JWT 管理、手机号验证 UI、非微信用户通知策略），拖慢交付。
>
> **当前执行**：H5 继续只做浏览、邀请函和消息中心抽屉内详情，不引入 H5 报名，也不单独做完整 H5 匹配系统页面。

#### 1.7.3 `/chat` xu对话（降级方案）

**URL 格式**：`https://xu.example/chat`

**页面定位**：H5 版 xu 对话，作为小程序不可用时的承接方案。
...
| **流式响应** | 通过 `fetch + SSE` 调用 Elysia `POST /ai/chat` |
...
| 场景 | 说明 |
|------|------|
| 小程序审核中 | 新版本提交审核期间，用户可通过 H5 继续使用对话入口 |
| 小程序故障 | 小程序出现 bug 时，H5 作为备用入口 |
| 非微信环境 | 用户在浏览器中想体验xu对话 |
...
| 对话 UI | AI SDK Elements | Vercel 官方组件，copy-paste 模式安装 |
...
### 1.9 活动全生命周期 (Activity Full Lifecycle)

> 修复当前活动生命周期“后半段空白”问题，补全报名 → 讨论 → 活动前 → 活动后 → 反馈的完整闭环。
...
- 如果活动满员，`join_activity` 不应直接硬失败；系统应优先承接为候补，并明确告诉用户当前仍在等待空位
- 报名按钮区域不得再引入“向发起人介绍一下自己”这类额外输入，除非后续某个明确领域能力真正消费该信息
...
- AI 产出的复盘正文会提炼为 `reviewSummary` 并写回 `user_memories`，后续再约、推荐和欢迎卡优先参考这份真实结果
...
- 首页不再把欢迎内容塞成 `Widget_Dashboard` 消息；欢迎态是页面级结构，不是消息级结构
- 首页只承担两件事：让用户愿意开始、让用户知道可以立刻从哪一句开始
- Welcome Stage 只保留：导航、Hero、3 条快捷入口、底部输入框
- 一旦用户真正进入聊天（发送第一条消息 / 点击快捷入口并触发响应），页面切到 **Conversation Stage**
- Conversation Stage 背景直接切到纯黑或近纯黑，让消息与 GenUI card 成为主角

**Conversation Stage 结构**：
...
| 界面块 | 说明 | 触发时机 |
|--------|------|----------|
| Widget_Dashboard | 历史兼容欢迎块 | 历史消息回放；不再承担首页首屏骨架 |
| Widget_Draft | 意图解析卡片 | 系统识别为"创建意图" |
| Widget_Share | 创建成功卡片 | 活动发布成功 |
| **Widget_Explore** | **探索卡片** | **系统识别为"探索意图"** |
...
| Widget_Error | 错误提示卡片 | 解析失败 |

#### 3.3.1 界面块数据架构

xu当前采用**结构化动作驱动的组件渲染架构**：工具返回结构化数据（`WidgetChunk`），前端根据 `messageType` 渲染对应的界面块组件。系统支持两种数据模式，根据场景自动切换。
...
│                    WidgetChunk 数据结构                       │
├─────────────────────────────────────────────────────────────┤
│  messageType: string          ← 界面块类型标识               │
│  payload: Record<string, any> ← 界面块数据                   │
...
> **当前执行逻辑**：从单一 NLP 识别升级为三层优先级系统，用来承接热词直达、结构化意图和兜底引导。
...
| 场景 | 用户输入 | 系统响应 |
|------|---------|-----------|
| 完全无关 | "今天天气怎么样" | "我是你的活动助理xu，可以帮你组局、找搭子、探索附近活动。试试这样说：'明晚观音桥火锅'、'周末想玩点啥'。" |
...
### 3.5 进场欢迎体验（Welcome Stage）

> **当前执行口径（v5.4）**：欢迎态首先是一个页面，而不是一条消息。它负责营造进入感、降低开口门槛，并把用户顺滑带入同一条对话主链。

#### 3.5.1 Welcome Stage 布局
...
- 只保留最必要的情绪和行动：导航、Hero、3 条快捷入口、底部输入框
- 不在首屏额外展示社交统计、偏好完善度、活动资产摘要
- 允许展示 1 个后端驱动的轻量 `Welcome Focus`，用于承接“活动反馈 / 招人进展 / 未完成意愿”
- 背景可以有轻微 Orb / 轻微动态氛围，但它必须服务文案，而不是抢文案
- 输入框是首页唯一明确 CTA，作用是让用户说出第一句话

#### 3.5.2 首页主文案

> **原则**：首页只保留一句主文案，不再使用动态问候和副文案，以免干扰第一句话输入。

| 场景 | 文案 |
|------|------|
| 默认首页 | "今天有什么想玩的？" |

**副文案原则**：默认不展示副文案。后续如需承接活动后反馈或招人进展，使用 `Welcome Focus` 的轻量 pill，而不是在 Hero 下方追加说明。

#### 3.5.3 Welcome Stage 的动作组织

Welcome Stage 不再以 `Widget_Dashboard` 为首页骨架，而是以 **3 条建议入口 + 输入框** 为首页主结构：

| 区块 | 当前作用 | 设计要求 |
|------|----------|----------|
| Welcome Focus | 承接一个最值得继续的事项 | 只允许 1 个小 pill；由 `/ai/welcome` 后端选择，不在前端推断 |
| Hero | 建立进入感 | 大字、强排版、低信息密度 |
| Quick Entries | 帮用户起第一句 | 每条是一句建议，不像菜单，不像 chip |
| Composer | 主 CTA | 弱化“发布”，强化“说第一句话” |
...
点击任意入口 → 直接发送该 prompt → 页面切入同一条对话主链。

**Welcome Focus 优先级**：

1. 活动后反馈：如果用户有已进入 `post_activity` 阶段的任务，优先询问“这次局怎么样？”
2. 招人进展：如果用户发起的活动仍缺人，提示继续推进招人结果
3. 未完成意愿：如果用户有未完成的找搭子 / 建局任务，提示继续处理

Welcome Focus 只解决“回来后接着做什么”的问题，不替代 Quick Entries，也不恢复首页统计卡。

当 Welcome Focus 命中活动后反馈时，3 条快捷入口临时切换为“挺顺利 / 一般 / 没成局”。点击后不走纯文本猜测，而是发送结构化动作 `record_activity_feedback`，将结果写入活动结果记忆和对应任务结果。

#### 3.5.4 Conversation Stage

> **原则**：欢迎态可以有情绪，聊天态必须让位给内容。

当用户发送第一条消息或点击任一快捷入口后：

- Hero 消失
- Quick Entries 收起
- 背景切到纯黑或近纯黑
- 消息流成为主角
- GenUI blocks 在纯黑舞台上承接结果、选择、填写与执行

**核心要求**：
- Welcome Stage 和 Conversation Stage 是同一页面的两种状态，不是两个割裂页面
- 进入聊天后不再保留大面积欢迎背景，避免与消息和 GenUI card 争抢注意力
...
| 旧工具感输入框 | 当前欢迎输入框 |
|---------------|----------------|
| 提示用户“你已经知道要干什么” | 邀请用户“先说第一句话也可以” |
| 像工具栏 | 像页面的一部分 |

**当前口径**：
- Welcome Stage 的输入框允许有轻微边界呼吸感（如 Border Glow）
- 这种呼吸感只是边界层，不是发光特效，不应抢 Hero 文案
- Conversation Stage 中输入框要收回到更稳定、更安静的深色形态
...
### 11.1 当前执行策略：Visitor-First + Action-Gated Auth

**三态用户定义（当前主流程）**：

| 用户状态 | 标识 | 核心能力 |
|---------|------|---------|
| **Visitor** | 无登录态 | 可浏览公开内容、探索附近、体验对话入口 |
...
## 13. 视觉设计：Editorial Dark (编辑感暗色)

### 13.1 设计灵感

当前视觉不再以“支付宝蓝 + 柔和科技卡片”为主，而是改为：

- **默认夜间模式**
- **黑白灰为主**
- **高级感主要来自留白、比例、字号层级、内容节奏**
- **Welcome Stage 允许非常克制的 Orb / 轻氛围背景**
- **Conversation Stage 直接切纯黑或近纯黑背景，让 GenUI card 成为主角**

H5 首页视觉参考更接近 React Bits 一类的暗色单页舞台，但只借：
- 黑白灰秩序
- 中央情绪锚点
- 干净按钮与输入框气质
...
| 用途 | 浅色模式 | 深色模式 |
|------|----------|----------|
| 页面背景 | #F5F7FA | #090909 / #0B0B0C |
| 卡片背景 | #FFFFFF | rgba(255,255,255,0.03) ~ rgba(255,255,255,0.06) |
| 主文字 | #1F2937 | rgba(255,255,255,0.96) |
| 次文字 | #6B7280 | rgba(255,255,255,0.42) ~ rgba(255,255,255,0.68) |
| 主 CTA | #111111 | #FFFFFF（白底黑字） |
| 轻边界 | rgba(0,0,0,0.08) | rgba(255,255,255,0.08) |
| Welcome 氛围层 | - | 允许极弱冷色偏（Orb / 背景层），但不可主导页面 |

### 13.3 卡片风格（Conversation Stage）

- 聊天中的 GenUI card 默认站在纯黑舞台上
- 卡片优先使用暗色半透明底和轻边界，而不是亮白玻璃卡
- 主按钮优先白底黑字；次按钮优先暗色描边
- 阴影只做层级，不做“科技感发光”

### 13.4 Welcome / Conversation 双态

- **Welcome Stage**
  - 默认夜间模式
  - 可有很克制的 Orb / 背景气氛层
  - Hero 文案是主角，输入框是主 CTA
- **Conversation Stage**
  - 背景切纯黑或近纯黑
  - 所有视觉注意力让位给消息与 GenUI
  - 更适合承接 richer cards、结果卡、表单卡、搭子卡
...
> **当前生效策略**：活动详情采用**统一详情页（Page-First 执行）**。对话入口负责发现与表达，报名/复制/克隆等确定性动作在详情页完成。
...
> **xu 是一个助理，而不是一个平台入口**。活动、搭子、群聊、复盘和推荐都应表现为 xu 的能力模块，而不是互相割裂的页面。
...
| 元素 | 文案 |
|------|------|
| 标题 | (系统生成的活动标题，如：🔥 观音桥老火锅，3缺1) |
| 副标题 (App 来源) | "xu - 今天有什么想玩的？" |
| 默认分享标题 | "xu - 你的 AI 活动助理" |
...
| 页面 | 路径 | 类型 | 说明 |
|-----|------|------|------|
| AI 主场 | `pages/chat/index` | 主包 | Welcome Stage + Conversation Stage，首页与对话主链合一 |
...
| **沉浸式地图** | `subpackages/activity/explore/index` | 分包 | 探索卡片展开 |
...
**用户承接要求**：
- 当前版本不再要求首页顶部额外展示 `join_activity / find_partner / create_activity` 的任务胶囊；首页继续以对话入口、欢迎卡、热词与探索为主
...
**这套动态界面块的核心价值**：根据用户意图返回更合适的界面，而不是简单地返回文本或跳转页面。
...
> **核心逻辑**：一个普通用户，最开始是 Joiner（参与者）。他点开了一张精美的邀请函，觉得"这卡片真酷"。当他下次自己想约饭时，如果能想起用 xu 发一个局，他就完成了从 Joiner 到 Creator 的跃迁。**这是 xu 最核心的增长引擎：体验即营销。**
...
| "怕卡片不够吸引人" | 草稿→发布转化率 | ≥ 60% | 系统生成草稿后确认发布的比例 |
...
**核心功能**：
- **极简指挥舱**：首页只看 3-4 个最关键数字与今天要优先推进的事项，不再追求大而全的数据中台
- **内容工作台**：围绕真实需求批量出稿、内容库沉淀、手工回填效果、沿赢家方向继续扩写
- **热词管理**：P0 层全局关键词配置、响应内容编辑、轻量命中/转化分析
- **组局盘面**：统一看活动盘面、待处理搭子意向和待跟进匹配
- **风险审核**：处理 AI 审核队列
- **用户举报**：处理用户主动提交的问题内容 / 问题对象

**Admin 信息架构**：
- **概览**：指挥舱首页，只看关键数字和今日优先处理
- **内容**：内容工作台内用 tabs 承接“内容 / 热词”，统一处理选题、出稿、效果回填和轻量热词分析
- **组局**：活动与搭子盘面，统一处理成局承接和人工跟进
- **风控**：风险审核与用户举报两张页面，避免把两类处理动作硬塞进同一屏
- **AI**：Playground 工作区内承接“调试 / 配置”，再加对话记录和用量统计
- **用户**：用户管理
- **设置**：系统设置

以上分组用于降低单人使用时的认知负担，但不引入额外的角色分层，也不通过隐藏真实能力来“瘦身”。
...
**xu当前的找搭子 = 即时搜索 + 明确下一步 + 可选异步意向池**

用户先表达“想找什么样的搭子”，系统先返回符合条件的候选结果；如果当下没有合适人选，或用户明确希望“xu继续替我盯着”，才创建意向、进入意向池并通过**微信服务通知**触达。
...
它们本质上都不是新产品，而是同一个 `find_partner` 主链里的不同语义形态：
- **本地搭子**：重庆本地、附近、下班后、周末
- **目的地同行**：一起去某个地方、某个活动、某个演出/音乐节
- **临时补位**：差一人、补位、三缺一

当前产品要求不是把它们拆成不同流程，而是让 Agent 先理解“时间 + 地点/目的地 + 场景类型 + 用户诉求”，再统一走搜索优先、下一步明确、必要时再入池的主链。
...
| 规则 | 说明 |
|------|------|
| 搭子类型 | 同大类匹配（运动 ↔ 运动、饭搭子 ↔ 饭搭子、音乐节/演出/展 ↔ 同去/同行） |
| 方便区域 | 区域可达或文本归一后相近 |
| 时间重叠 | 时间段有交集；保留原始自由文本如“周6”“这周六”用于后续追问与摘要 |
...
此前我们引入了 Chat Tool Mode，但找搭子场景更适合使用**服务通知**而非群内系统消息，原因如下：
...
- 消息中心卡片和详情态文案必须明确告诉用户“这是刚才那条找搭子任务的继续”，避免像跳进另一套新流程
...
> 当前执行补充：`find_partner` 的首轮理解不再只依赖“重庆片区 + 固定搭子词表”。像“泸州音乐节有人去吗”“周6平顶山有没有人”这类真实群聊表达，也必须先被当成同一条找搭子主链理解，再决定是本地搭子、目的地同行还是临时补位。
  ↓
...
- 搜索结果区必须明确说明“现在还处于先搜一下阶段”，并把“发起搭子邀请 / 让xu帮我问 / 继续帮我留意”做成可区分的下一步分叉
- 结果页下一步动作：
  - `登录后搭一下`
  - `登录后让xu帮我问问`
  - `登录后继续帮我留意`
...
**技术实现**：
- 存储在 `user_memories` 表（自然语言内容 + metadata + embedding）
- 通过 LLM 从对话历史中提取偏好
...
| W2-2 | 复用 `/ai/chat`（action/text 预填） | `apps/miniprogram/subpackages/activity/detail/index.ts`、`apps/miniprogram/pages/chat/index.ts` | 已支持“我也组一个”入口与主场预填回跳 |
...
| 术语 | 说明 |
|------|------|
| **Brand_Slogan** | **"今天有什么想玩的？" - 核心品牌口号** |
| **Chat_First** | **首页即对话，所有功能封装在对话流中，不是 UI 风格而是产品定位** |
| **Generative_UI** | **根据意图动态生成最合适的 Widget** |
| **Memory_Layer** | **记忆层，AI 记住用户偏好，下次推荐更准** |
| Chat_Stream | 对话流，首页核心区域 |
| AI_Dock | 对话输入区；在首页 Welcome Stage 中作为主 CTA，在 Conversation Stage 中作为持续输入入口 |
| **xu (xu)** | **产品内的助理角色名** |
| Widget_Dashboard | 历史兼容欢迎块；保留历史消息回放，不再承担首页首屏骨架 |
| Widget_Draft | 意图解析卡片（创建场景） |
| Widget_Share | 创建成功卡片 |
| **Widget_Explore** | **探索卡片（探索场景）** |
| **Explore_Map_Page** | **沉浸式地图页，全屏可交互地图** |
| **Invitation_Card** | **活动邀请函，用于分享活动信息** |
...
| Share_Card | 微信原生分享卡片 |
| Editorial_Dark | 当前首页视觉方向：默认暗色、黑白灰排版、Welcome Stage 有克制氛围、Conversation Stage 为纯黑内容舞台 |
| Soft_Card | 历史术语；旧的亮底卡片语言，当前首页与主聊天态不再使用 |
| Cornflower_Blue | 历史主色术语；当前首页与主聊天态不再以此作为视觉主轴 |
| **Auth_Sheet** | **半屏授权弹窗，替代原生手机号授权弹窗** |
| **Share_Guide** | **分享引导蒙层，发布成功后引导用户分享** |
| **Activity_Preview_Sheet** | **活动预览浮层，地图页点击 Pin 时显示** |
| **Thinking_Bubble** | **思考气泡，表示系统处理中** |
...
| **Scenario_Split** | **场景隔离，xu与粗门的差异化定位边界** |
| **Small_C** | **普通用户，区别于 Big C（俱乐部主理人、商家）** |
| **Long_Tail** | **长尾场景，80% 的生活/社交活动（xu主攻）** |
| **Partner_Matching** | **找搭子能力，Agent 自然识别找搭子意图并帮用户匹配** |
| **Partner_Intent** | **搭子意向，用户想参加某类活动的意愿表达** |
| **Temp_Organizer** | **临时召集人，匹配成功后负责确认并转为活动的用户** |
| **Widget_AskPreference** | **单步偏好追问卡片，适合补 1 个缺失槽位** |
| **GenUI_Form** | **正文下方的结构化表单卡片，适合一次收集多项偏好** |
| **Global_Keywords** | **全局关键词，P0 层热词库，直接返回预设内容无需 NLP** |
| **Hot_Chips** | **历史首页热词入口；当前 Welcome Stage 首屏不再作为主结构，保留给局部场景或历史兼容** |
| **Dual_Mode_Drive** | **双模驱动，Global Commands（快车道）+ Natural Language（慢车道）** |
| **Context_Agnostic** | **上下文无关，Agent 统一响应原则，不关心用户来源** |
| **Visitor_First** | **默认开放浏览：未登录用户可浏览对话/详情/探索，先体验再转化** |
| **Action_Gated_Auth** | **动作闸门：报名/发布/联系方式等高价值动作统一要求登录 + 手机号绑定** |
| **OpenID** | **微信用户标识，用于登录身份关联；当前主流程不以 OpenID 直接放开报名动作** |
| **Activity_Clone** | **活动克隆，[👯‍♂️ 我也组一个] 功能，快速创建类似活动** |
| **Service_Notifications** | **服务通知，微信官方通知渠道，用于找搭子匹配成功等场景** |
| **Asynchronous_Intent_Pool** | **异步意向池，找搭子的核心模型，后台持续匹配** |
| **Admin_Commands** | **管理员命令，前端可见的快速配置系统，用于直播间运营** |
| **Activity_Discussion** | **活动讨论区，基于 WebSocket 的实时通讯功能，仅限活动参与者** |
| **Content_Security** | **内容安全检测，接入微信 msgSecCheck API 进行敏感词过滤** |
| **Offline_Notification** | **离线通知，用户不在线时通过客服消息或服务通知推送消息** |
| **H5_Invitation** | **H5 活动邀请函，URL 格式 `https://xu.example/invite/{activityId}`，SSR 渲染 + OG 标签，支持跨平台分享** |
| **Theme_System** | **活动主题系统，为每个活动分配视觉主题（动态背景 + 配色 + 文字效果）** |
| **ThemeConfig** | **主题配置 JSON，存储邀请函背景与配色参数（由预设或配置工具生成）** |
| **OG_Tags** | **Open Graph 标签，社交平台分享时展示标题、描述、图片的 HTML meta 标签** |
| **React_Bits** | **开源 React 动效库；当前 H5 仅作为 welcome 背景/输入框 wrapper 的视觉包装层，不替代 ai-elements 聊天主轴** |
| **Preset_Theme** | **预设主题，6 种内置主题：aurora（极光）、party（派对）、minimal（简约）、neon（霓虹）、warm（暖色）、sport（运动）** |
| **AI_Elements** | **Vercel AI SDK Elements (https://elements.ai-sdk.dev)，copy-paste 模式安装的对话 UI 组件** |
| **Eden_Treaty** | **Elysia 类型安全 HTTP 客户端（admin 已使用；web 侧当前阶段为 Fetch/Transport，后续可接入）** |
| **Post_Activity_Flow** | **活动后流程，活动结束后自动触发的反馈收集和 AI 跟进** |
| **Agent_Task_Runtime** | **任务运行时，持续拥有同一件事的中枢对象；当前覆盖 `join_activity`、`find_partner`、`create_activity`** |
| **Join_Activity_Task** | **“帮用户找到并加入一个局”的连续任务，从探索到报名、讨论区、活动后跟进** |
