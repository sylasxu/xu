...
> 增长模块的"文案工厂"（`POST /growth/poster/generate`）保留，用于生成小红书 / 抖音 / 微信风格内容，与海报截图无关；生成结果除正文外，还应包含可直接用于首图的短文案与配图提示词，并支持基于平台与内容类型生成可点击填入的 AI 主题建议；生成文案必须避免站外引流表达（如加群、私信、扣 1、主页联系等）。
...
>
> **v5.1 决策点（历史记录）**：曾评估是否根据 H5→小程序转化率决定是否加 H5 轻量 RSVP（手机号登录）。
>
> **v5.3 当前执行**：H5 继续只做浏览、邀请函和消息中心抽屉内详情，不引入 H5 报名，也不单独做完整 H5 匹配系统页面。
...
## 11. 延迟验证策略：Visitor-First（当前）与 Guest-First（v4.8 历史归档）
...
### 11.2 Guest-First 策略 (v4.8 历史归档，非当前主流程)

> 以下 11.2 ~ 11.9 内容保留为 v4.8 历史方案记录，用于追溯讨论背景；**不作为当前版本的实现约束和验收标准**。
...
### 11.4 OpenID 报名流程 (v4.8 历史归档，非当前主流程)
...
### 11.6 手机号绑定触发时机 (v4.8 历史归档，非当前主流程)
...
### 11.7 Guest 转 Registered 流程 (v4.8 历史归档，非当前主流程)
...
### 11.8 数据库设计 (v4.8 历史归档，非当前主流程)

> 当前落库以 `@juchang/db` 为准：`users` 表不存 `isGuest` 实体字段（通过 `phoneNumber === null` 进行逻辑态判断）；`participants` 当前不含 `group_openid/joinedVia` 字段。若未来重启 Guest-First 实验，需先完成 TAD → DB Schema → API 的单向变更。
...
## 13.5 活动详情页交互策略：当前执行与 v4.8 历史方案
...
> **历史归档说明**：以下 13.5.1 ~ 13.5.7 保留为 v4.8 Skyline 双模式方案记录，用于追溯上下文；**不作为当前版本实现约束与验收标准**。
...
- 已覆盖用户可见承接：首页需展示“你刚才那件事，我还在盯着”的任务胶囊，而不是只把 runtime 藏在后端
...
**首页承接要求（v5.4 第三阶段）**：
- 当 authenticated 用户存在未完成的 `join_activity / find_partner / create_activity` 任务时，首页顶部应出现任务胶囊
- 任务胶囊至少展示：任务类型、当前阶段、当前摘要、推荐下一步动作
- 用户点“继续”时，应优先执行 runtime 提供的推荐动作，而不是前端自己重新猜这件事该去哪
- 消息中心与讨论区不要求重做成任务中心，但必须明确表现为同一条任务的后续承接面，而不是新的孤立页面
...
> **v5.4 当前执行**：找搭子的首轮承接不再默认等价于“创建意向并入池”，而是先按用户偏好做一次**即时搜索**，返回可浏览的搭子候选结果；只有当用户明确选择“和这个人搭一下 / 帮我问问 / 继续帮我留意”时，才进入登录约束与后续社交动作。
>
> **v4.8 历史概念**：找搭子的长期承接仍然保留**异步意向池（Asynchronous Intent Pool）**能力，用于“继续帮我留意”这类持续代找场景，通过后台持续匹配与服务通知触达用户。
...
| **Guest_Mode** | **~~历史实验策略~~：基于 OpenID 的轻量级报名（v4.8 方案），当前不纳入主流程** |
...
| **AI_Poster_Generator** | **~~AI 海报生成器~~ (v5.0 废弃，已被 H5 邀请函替代，poster 模块已移除)** |
