...
> 增长模块的"文案工厂"（`POST /growth/poster/generate`）保留，用于生成小红书风格文案，与海报截图无关。
...
> **v5.3 策略对齐（当前生效）**：主流程采用 **Visitor-First + Action-Gated Auth**。未登录用户可浏览和试聊，涉及写入和社交风险动作时再要求登录/绑定手机号。

### 11.1 当前执行策略：Visitor-First + Action-Gated Auth (v5.3)
