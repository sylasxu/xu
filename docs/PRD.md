...
| 字段 | 来源 | 说明 |
|------|------|------|
| 参与活动数 | `socialProfile.joinedActivities` | 由 `participants` 实时聚合的累计参与数量 |
| 发起活动数 | `socialProfile.hostedActivities` | 由 `activities` 实时聚合的累计发起数量 |
| 偏好完善度 | `user_memories` | 根据长期记忆中的偏好与地点线索计算 |
...
**偏好数据存储**：保存到 `user_memories` 表，由对话链路自动学习与提炼
...
> **当前交互目的**：在输入框上方显示可点击的热词胶囊，降低首次输入门槛，并为高频场景提供直达入口。
