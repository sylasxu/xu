...
聚场不是从抽象 AI 往场景里找落点，而是从这些高频、口语化、结果导向很强的真实表达出发，设计一个能持续接住它们的组局助手。
...
- 用户一旦进入聚场，默认已经脱离微信群公开发言压力，因此报名动作不应再附带自我介绍、社交表演或额外说明
...
| 临时起意 (今晚去哪喝一杯？谁有空？) | 🟢 **聚场 胜** | 唯快不破。对话入口更适合临时起意和快速摇人 |
...
聚场当前采用**结构化动作驱动的组件渲染架构**：工具返回结构化数据（`WidgetChunk`），前端根据 `messageType` 渲染对应的界面块组件。系统支持两种数据模式，根据场景自动切换。
...
**副文案原则**：
- 只做一行非常轻的行动邀请，例如“附近有新局，想直接看看吗？”
- 不解释功能，不灌输“我是 AI”，不提示“粘贴群聊”
...
- 搜索结果区必须明确说明“现在还处于先搜一下阶段”，并把“发起搭子邀请 / 让小聚帮我问 / 继续帮我留意”做成可区分的下一步分叉
- 结果页下一步动作：
  - `登录后搭一下`
  - `登录后让小聚帮我问问`
  - `登录后继续帮我留意`
...
| **H5_Invitation** | **H5 活动邀请函，URL 格式 `https://juchang.app/invite/{activityId}`，SSR 渲染 + OG 标签，支持跨平台分享** |
