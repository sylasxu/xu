...
| **对话入口** | 首页以对话作为主要入口，能力围绕对话流和结果卡片展开 |
...
// Content (内容运营领域)
POST /content/generate              // 按平台（小红书 / 抖音 / 微信）与内容类型单次生成内容，并在本地做规范化清洗后落库标题/正文/标签/首图文案/首图配图提示词
...
H5 邀请函是当前唯一的分享承接页，负责外部曝光、社交氛围营造与回流到小程序。

**当前约束**：
- 邀请函只承担浏览、预览与跳转，不承接报名动作
- 邀请函展示标题、地点、时间、参与人数、讨论区预览与状态信息
- 非微信环境展示小程序码，微信内使用 URL Scheme / deeplink 承接
- 同一活动的邀请函内容允许缓存，优先保证打开速度与分享稳定性
...
apps/web 是面向用户的 H5 Web 应用，基于 Next.js 15 构建，当前主要提供跨平台活动邀请函和 Web 侧对话入口，让活动链接能在微信外先被浏览，再把用户引导回微信内继续参与。
...
| `/chat` | CSR | 无需认证 | Web 对话入口，小程序降级方案 |
