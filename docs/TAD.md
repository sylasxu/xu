...
│   ├── intent-classify.ts # P1+P2 意图分类（三层漏斗级联）
│   ├── user-profile.ts   # 用户画像注入 (memoryContext → System Prompt)
│   ├── semantic-recall.ts # 语义召回历史 (pgvector + rerank)
...
│   ├── llm-classifier.ts     # P2 LLM Few-shot 分类器
...
│   ├── preference-signal.ts # 偏好信号前置检查
│   ├── semantic.ts       # 记忆向量检索与相似度计算
...
P1: 规则组合识别（Feature Combination）
  ...
  ▼
P2: LLM Few-shot 分类器（LLM Few-shot）
  │ 成功 → 返回分类结果
...
- `P1 Feature Combination`：优先承担主意图识别
- `P2 LLM Few-shot`：作为复杂表达与低置信场景兜底
