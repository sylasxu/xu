...
| `GET /ai/tasks/current` | 获取当前用户仍在推进中的任务快照 | 供 runtime 联调、回归与首页/后续承接面读取任务阶段；当前 H5 首页已直接消费该接口并根据返回的 `homeState + primaryTaskId` 渲染主状态卡 |
...
- 报名成功后，任务推进到 `joined`
- 小程序进入讨论区后，调用 `POST /ai/tasks/discussion-entered`，任务推进到 `discussion`
...
- 通过 `GET /ai/tasks/current` 拉取当前 open task snapshots，并消费 `homeState + primaryTaskId` 决定首页主状态卡
