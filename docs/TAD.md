...
| `content` | `/content` | **内容运营**：传单式内容生成、内容库管理、效果追踪 (v5.1 从 Growth 迁移) |
...
POST /ai/generate/content       // AI 生成内容（海报文案、小红书笔记）(v5.1 从 Growth 迁移)
...
GET  /analytics/trends              // 趋势洞察（高频词/意图分布）(从 Growth 迁移)
GET  /analytics/content-performance // 内容效果分析 (从 Growth/content 迁移)
GET  /analytics/metrics             // 业务指标（J2C、成局率、留存率等）
GET  /analytics/platform-overview   // 平台概览（实时概览/AI 健康度/异常警报）

// Content (v5.1 新增：内容运营领域，从 Growth 迁移)
...
GET  /content/analytics             // 内容效果统计

// Growth (v4.5 新增：增长工具，v5.1 已迁移到各领域，保留兼容层)
...
> 说明：v4.8 中“OpenID 直接报名”的 Guest-First 为历史方案，当前版本未纳入主流程；找搭子当前采用“Search-First + Optional Pool”，搜索本身不触发认证闸门。
...
| `GET /ai/tasks/current` | 获取当前用户仍在推进中的任务快照 | 首页据此渲染“持续帮你推进”的任务胶囊，并执行 runtime 推荐动作 |
...
- 首页通过 `GET /ai/tasks/current` 拉取 open task snapshots，把 runtime 的“当前阶段 + 推荐动作”直接暴露给用户
- 当 `find_partner` 任务进入 `match_ready` 时，任务快照的推荐动作可附带 `matchId` 聚焦信息；首页切到消息中心后应直接承接到对应待确认匹配，而不是让用户重新找一遍
- 消息中心顶部先按 `needs_action / in_progress` 分组展示 open task snapshots，系统通知退居 `结果更新`；讨论区通过轻量阶段提示表明自己是 `discussion` 阶段承接面，而不是独立流程
...
type StructuredActionType = 
  ...
  | 'submit_partner_intent_form' // v5.3 兼容动作，逐步迁移到 search_partners / opt_in_partner_pool
...
- Transport：待迁移到 `/ai/chat` GenUI SSE（当前 Admin Playground 仍在旧链路重构中）
...
- **CP-56**: `GET /ai/tasks/current` 只能返回当前用户仍处于 open status 的任务，并按 `updatedAt` 倒序输出给首页承接层
