...
| `GET /ai/tasks/current` | 获取当前用户仍在推进中的任务快照 | 供 runtime 联调、回归与后续承接面读取任务阶段；当前版本不要求首页直接展示任务胶囊 |
...
- `GET /ai/tasks/current` 保留为 runtime 快照查询接口，供联调、回归与后续承接面接入；当前版本不要求首页直接拉取并展示 open task snapshots
- 当 `find_partner` 任务进入 `match_ready` 时，任务快照的推荐动作可附带 `matchId` 聚焦信息；消息中心或待确认匹配详情应直接承接到对应匹配，而不是让用户重新找一遍
- 消息中心、pending match 详情、讨论区与 post_activity follow-up 必须明确表现为同一条任务的后续承接面；当前不再要求首页或消息中心额外渲染任务胶囊
...
- **CP-56**: `GET /ai/tasks/current` 只能返回当前用户仍处于 open status 的任务，并按 `updatedAt` 倒序输出给承接/联调消费层
