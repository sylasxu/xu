...
| 原则 | 说明 |
|------|------|
| **Agent-First（内部实现）** | `Agent` 只代表系统内部的持续任务运行时、上下文恢复与结果闭环；它是技术实现，不是对外品牌表达 |
| **Assistant-Facing（对外角色）** | 用户看到的是“组局助手小聚”，不是 `agent runtime`、不是任务系统、不是多页面流程接力 |
...
**内部产品原型（用于架构判断，不用于对外传播）**：

- 小聚是“高频组局群主日常工作的数字化身”
- 技术上要复刻的不是抽象大模型人格，而是“听懂群聊口语表达、判断这是找局/找搭子/组局/活动后续上、并把它持续推进到结果”的组织能力
- 因此 `join_activity`、`find_partner`、`create_activity` 三条任务运行时，本质上是在数字化“群主如何接话、接事、接后续”
...
### 4.5.1 Agent Task Runtime 表（v5.4 新增）

> v5.4 第三阶段同时为 `join_activity`、`find_partner` 与 `create_activity` 建模，目标不是替代 conversations / notifications / discussion，而是在它们之上增加一个“持续拥有任务”的真源对象。
...
| `users` | `/users` | 用户资料管理、**用户统计** (v5.1 新增) |
| `activities` | `/activities` | 活动 CRUD、报名退出、**附近搜索**、**公开详情**、**活动统计** (v5.1 新增) |
| `participants` | `/participants` | 参与者管理 |
| `chat` | `/chat` | 活动群聊消息 (activity_messages 表)，**WebSocket 讨论区** (v4.9) |
| `ai` | `/ai` | AI 解析 (SSE)，**意图分类**，**对话历史管理**，**AI 内容生成** (v5.1 扩展) |
...
// AI (v3.9 扩展：AI 解析 + 对话历史 + 会话管理 + v5.1 内容生成)
POST /ai/chat                   // AI 对话统一网关（GenUI-only contract）
...
POST /ai/generate/content       // AI 生成内容（海报文案、小红书笔记）
...
// Content (v5.1 新增：内容运营领域)
POST /content/generate              // 按平台（小红书 / 抖音 / 微信）与内容类型生成内容，先产出初稿，再做“传单式 / 组织号”二次改写与清洗，最终落库标题/正文/标签/首图文案/首图配图提示词
...
| 个人资产读写 | `GET /ai/conversations`、`GET /ai/conversations/:conversationId/messages`、`POST /ai/conversations` | 必须登录 | 仅本人或 admin 可访问 |
...
### 5.3 AI Chat Gateway - 统一意图入口 (v5.3)

> v3.2 的 `/ai/parse` 意图解析能力已完全并入 `/ai/chat`。当前不存在独立 parse 接口；所有 text / action 输入统一通过 GenUI Chat Gateway 进入同一条主链路。
...
type GenUIChatRequest = {
  ...
  trace?: boolean; // stream=true 时是否返回 trace 调试事件；false 时 API 自动返回精简 SSE
  stream?: boolean;
}

type GenUIChatResponse =
  | GenUITurnEnvelope         // stream=false
  | GenUIStreamEvent[];       // stream=true (SSE)
```
...
#### 5.3.1 Agent Task Runtime（v5.4：join_activity + find_partner + create_activity）

`/ai/chat` 继续作为统一入口，但对 `join_activity`、`find_partner` 与 `create_activity` 增加一层任务运行时持久化：
...
- `followUpMode='review'` 的 AI 对话会在 completed 活动场景下把生成结果提炼后写入 `activityOutcomes.reviewSummary`
...
**支持的模型 (v4.8 OpenAI 兼容网关 + Qwen + DeepSeek 路由)**：

| 提供商 | 模型 | 用途 |
|--------|------|------|
| **OpenAI Compatible** | `gpt-5.4` | **主力 Chat / 深度思考 / Agent**（可通过 `OPENAI_BASE_URL` 接 sub2api 等兼容网关） |
...
export function getModelByIntent(intent: 'chat' | 'reasoning' | 'agent' | 'vision'): LanguageModel {
  switch (intent) {
    case 'chat':      return openai('gpt-5.4');      // 日常对话
    case 'reasoning': return openai('gpt-5.4');      // 深度思考
    case 'agent':     return openai('gpt-5.4');      // Tool Calling
...
- **P0-找搭子搜索先行**：`apps/api/src/modules/ai/user-action/handler.ts` 将找搭子表单提交从“默认入池”拆为“先搜索、后决定是否入池”
- **P0-搭子搜索结果卡**：新增 `widget_partner_search_results`，由 API 返回候选列表、匹配理由与登录前可见摘要，小程序/H5 统一消费
...
- **P1-post_activity 回流**：小程序 `pages/message/index.ts`、`subpackages/activity/confirm/index.ts` 与 H5 `message-center-drawer.tsx` 在 post_activity / 再约承接时透传 `activityId + followUpMode`，点击后直接触发 AI 对话反馈 / 再约 prompt，并将 review 结果回写 `activityOutcomes.reviewSummary`
...
- AI 对话统一调用 `POST /ai/chat`（GenUI SSE 响应）
...
- 通过 `fetch` + SSE 调用 `POST /ai/chat` 端点（`stream=true`）
...
- 页面状态由 `apps/miniprogram/src/stores/chat.ts` 维护：`text/action` 输入 + `context.lat/lng/activityId/followUpMode` -> `/ai/chat`（GenUI payload） -> `turn.blocks` 渲染
...
### 9.1 Admin Cockpit Redesign (v5.3)

**设计理念**：从传统 CRUD 管理系统转型为 Solo Founder 的 AI Cockpit + Ops Workbench
...
| **导航** | 功能平铺 | 分层聚焦（指挥舱/AI Ops/安全/内容运营） |
| **AI 调试** | 分散的配置页 | 统一 Playground |
| **运营视图** | 无 | 活动与搭子统一入口 + 内容运营工作台 |
