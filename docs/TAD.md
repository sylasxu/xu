...
| 原则 | 说明 |
|------|------|
| **任务运行时** | 用持续任务对象记录同一件事的阶段、结果和下一步 |
| **状态首页** | 首页不是活动广场，也不是纯 chat；首页负责承接当前最需要继续推进的事，对话主舞台负责把这件事办下去 |
| **界面块返回** | 根据意图返回不同的界面块（创建、探索、结果、追问），而不是固定单页流程 |
| **上下文记忆** | 记录明确表达过的偏好与历史上下文；对话历史持久化到 conversations 表 |
...
│   │   │   ├── activities/
│   │   │   │   └── [id]/
│   │   │   │       └── page.tsx    # SSR 活动详情分享态（OG Tags + 任务舞台）
│   │   │   └── chat/
...
// activities 表（含 theme/themeConfig 字段）
...
### 4.5.1 任务运行时表（v5.4 新增）

> 当前同时为 `join_activity`、`find_partner` 与 `create_activity` 建模，目标不是替代 conversations / notifications / discussion，而是在它们之上增加一个“持续拥有任务”的真源对象。
...
export const agentTasks = pgTable('agent_tasks', {
  ...
  slotSummary: jsonb('slot_summary').$type<Record<string, unknown>>(),
...
**使用原则**：
- 只跟踪“帮用户找到并加入一个局”与“帮用户找到搭子并承接异步匹配”这类连续任务，不把所有聊天都强行建成任务
- `conversations` 负责“说过什么”，`agent_tasks` 负责“这件事推进到哪了”
- `slotSummary` 虽然仍存 `jsonb`，但运行时已按任务类型收敛成显式 context shape，而不是自由 JSON 漫游：
  - `join_activity`: `location + activityType + timePreference + vibe + budget + semanticQuery + selectedActivityId`
  - `find_partner`: `activityType + locationHint + timePreference`
  - `create_activity`: `title + type + locationName + startAt + maxParticipants`
- `conversation_messages.taskId` 用于把 turn 级消息快照回挂到对应任务，便于任务复盘、运营分析和跨入口恢复
...
`global_keywords` 保留的定位是“运营层的补充信号”，而不是产品主流程的中心路由。它可用于：

- 为内容工作台、自媒体引流、局部快捷入口提供可配置关键词
- 在 `/ai/chat` 入口前补充极轻量的业务信号或运营直达
- 记录命中与转化，用于复盘哪些关键词值得继续运营
...
| `hot-keywords` | `/hot-keywords` | **Internal / Ops**：关键词运营与补充信号管理 |
...
GET  /activities/nearby   // 获取附近活动（支持探索卡片和 keyword）
GET  /activities/stats    // 获取活动统计 (概览/类型分布)
PATCH /activities/:id/status  // 更新活动状态
DELETE /activities/:id    // 删除活动
POST /activities/:id/join // 报名活动（含系统消息与通知）
POST /activities/:id/quit // 退出活动

// Participants
GET  /participants/activity/:id          // 获取活动参与者列表
POST /participants/confirm-fulfillment   // 发起人提交履约确认
POST /participants/self-feedback         // 参与者提交活动后轻反馈（写入 task outcome + memory）
...
// Chat (活动群聊 + WebSocket 讨论区)
GET  /chat/:activityId/messages  // 获取消息列表
POST /chat/:activityId/messages  // 发送消息
WS   /chat/:activityId/ws        // WebSocket 连接
POST /chat/:activityId/report    // 举报消息

// Poster（已废弃，分享承接已统一收口到公开详情 / 历史兼容分享页）
// POST /poster/generate  ← 已移除

// AI (AI 解析 + 对话历史 + 会话管理)
POST /ai/chat                   // 对话统一 SSE 主链（界面块协议）
GET  /ai/tasks/current          // 当前用户任务快照（homeState + primaryTaskId + items[]）
POST /ai/tasks/join-auth-gate   // H5 活动详情报名被动作闸门打断时，记录可恢复 join_activity 任务
POST /ai/tasks/discussion-entered // 标记报名后已进入讨论区，推进 join_activity 到 discussion
GET  /ai/conversations          // 获取 AI 会话列表（显式 userId）
GET  /ai/conversations/:conversationId/messages // 获取指定会话消息
DELETE /ai/conversations        // 清空对话历史 (新对话)
GET  /ai/sessions               // 获取会话列表 (Admin 对话审计)
GET  /ai/sessions/:id           // 获取会话详情
DELETE /ai/sessions/:id         // 删除会话

// Hot Keywords（Internal / Ops）
...
POST /content/topic-suggestions     // 按平台与内容类型生成可直接填入的 AI 主题建议，默认偏“需求识别 + 组织入口”方向
GET  /content/library               // 内容库列表
GET  /content/library/:id           // 笔记详情
DELETE /content/library/:id         // 删除笔记
PUT  /content/library/:id/performance // 回填效果数据
GET  /content/analytics             // 内容效果统计（主路径，支持 contentType / startDate / endDate 查询）
```
...
- Action-Gated Auth：对“建立连接、持续代找、写入与社交风险”动作执行认证闸门（登录 + 手机号绑定）
...
| 找搭子即时搜索 | `POST /ai/chat`（`find_partner` / `search_partners`） | 可选认证 | Visitor 可填写偏好并查看搜索结果，不默认入池 |
| 搭子连接动作 | `POST /ai/chat`（`connect_partner` / `request_partner_group_up`） | 必须登录 | 明确社交动作，先登录再继续；必要时后续补手机号绑定 |
| 持续代找/入池 | `POST /ai/chat`（`opt_in_partner_pool` / `createPartnerIntent`） | 登录 + 手机号绑定 | 涉及意向写库与后续通知触达 |
| 个人资产读写 | `GET /ai/conversations`、`GET /ai/conversations/:conversationId/messages`、`DELETE /ai/conversations` | 必须登录 | 仅本人或 admin 可访问 |
...
> 说明：找搭子当前采用“Search-First + Optional Pool”，搜索本身不触发认证闸门。
...
| `POST /notifications/pending-matches/:id/confirm` | Temp_Organizer 确认成局 | 创建活动后返回 `activityId`，前端可直接跳转到活动详情 / 分享承接面 |
...
| `POST /participants/self-feedback` | 参与者提交活动后轻反馈 | 返回 `nextAction`，并把参与者真实反馈写入 task outcome 与 memory |
| `POST /participants/rebook-follow-up` | 记录活动后"去再约" | 为当前用户的该次活动 outcome 写入 `rebookTriggered=true` |
| `POST /ai/tasks/join-auth-gate` | H5 活动详情报名被登录 / 绑手机号打断时记录挂起点 | 创建或更新 `join_activity` 任务为 `auth_gate / waiting_auth`，保存 `pendingAction` 与 `activityId` |
| `POST /ai/tasks/discussion-entered` | 标记已进入讨论区 | 依据 `activityId` 将对应 `join_activity` 任务推进到 `discussion` |
| `GET /ai/tasks/current` | 获取当前用户仍在推进中的任务快照 | 返回 `homeState + primaryTaskId + items[]`；`items[]` 可携带 `attentionLevel`，供 H5 首页、侧边抽屉、消息中心和小程序任务面板选择主状态 |
...
### 5.3 入口层：统一对话入口 (`/ai/chat`)

> v3.2 的 `/ai/parse` 意图解析能力已完全并入 `/ai/chat`。当前不存在独立 parse 接口；所有 `text / action` 输入统一通过对话入口进入同一条主链路。

```typescript
type GenUIChatRequest = {
  ...
  trace?: boolean; // 是否返回 trace 调试事件；默认返回精简 SSE
}

type GenUIChatResponse = GenUIStreamEvent[]; // SSE，response-complete 事件内携带完整 envelope
```
...
- `trace`：客户端只需声明是否需要 trace；H5 / 小程序传 `false`，Admin 按需传 `true`，API 内部自动决定 SSE 是否保留 trace 相关事件与冗余 envelope
- 输出统一为 `response.blocks`，前端不再消费旧 parse widget 协议

**SSE / GenUI 协议约束清单（v5.3 收口）**：
- `/ai/chat` 的主输出是 `text` block；`choice / list / form / cta-group / alert` 只承接正文之后的选择、结果、填写与执行
- 当一轮回复同时包含解释文本与结构化交互时，SSE 必须先推送 `text` block，再推送对应的 structured block；H5 / 小程序前端按 block 到达顺序直接渲染
- 生成式 UI 不得重复承担正文解释职责；正文负责“说清楚”，结构化 block 负责“承接动作”
- H5 / 小程序前端不得再根据中文文案内容猜测“这段是不是重复”“这是不是位置问题”“该不该隐藏标题”；这些展示语义必须由 API 通过 block `meta` 明确表达
- block `meta` 是唯一展示语义来源；当前已收口字段包括：`choicePresentation`、`choiceInputMode`、`choiceQuestionType`、`choiceShowHeader`、`ctaGroupPresentation`、`ctaShowHeader`、`listPresentation`、`listShowHeader`、`formShowHeader`
- 前端只允许做协议级兜底：未知字段/老数据时走安全默认样式；禁止基于自然语言正文、question 文本或按钮标签做启发式布局判断
- 如果某轮没有 structured block，前端只渲染 text；如果存在 structured block，前端只负责在 text 之后换行渲染，不得重排为“先按钮后正文”

#### 5.3.1 任务运行时（主流程三条任务链）

`/ai/chat` 是统一入口，并为三条主流程增加任务运行时持久化：

- `join_activity`：帮用户找到并加入一个局
- `find_partner`：帮用户找搭子、同行或补位
- `create_activity`：帮用户把一个局发出去

| 运行时对象 | 作用 | 不替代什么 |
|-----------|------|-----------|
| `agent_tasks` | 记录“这件事现在推进到哪了” | 不替代 `conversations` |
| `agent_task_events` | 记录关键阶段推进与真实结果 | 不替代 `activity_messages` / `notifications` |

**推进规则**：
- authenticated 用户在“探索附近活动”并进入 join 语义时，可创建或更新 `join_activity` 任务
- 用户选择某个活动报名时，任务推进到 `action_selected`
- 被登录 / 手机号绑定拦截时，任务推进到 `auth_gate`，状态变为 `waiting_auth`
- H5 活动详情页直接报名被登录 / 手机号绑定拦截时，必须通过 `POST /ai/tasks/join-auth-gate` 写入同一套 `join_activity` 挂起语义；本地 `sessionStorage` 只作为页面内恢复兜底，不能替代服务端任务
- 报名成功后，任务推进到 `joined`
- H5 / 小程序进入讨论区后，调用 `POST /ai/tasks/discussion-entered`，任务推进到 `discussion`
- 活动后从消息中心 / AI follow-up 进入复盘或再约时，依据 `activityId + followUpMode` 推进到 `post_activity`
- 活动完成通知创建前，应确保参与者存在可挂回的 `join_activity` 任务；`post_activity` 通知、消息中心卡和 AI follow-up 应尽量共享同一个 `taskId`
- 当履约确认、复盘摘要或再约意愿写回成功时，任务记录 outcome 并推进到 `done`
- authenticated 用户触发 `find_partner` 后，任务推进到 `preference_collecting`
- 提交 `submit_partner_intent_form` 并成功写入 `partner_intents` 后，任务推进到 `intent_posted / awaiting_match`
- 真正创建 `intent_matches` 时，任务推进到 `match_ready`，而不是等用户自己再说一遍
- `confirm_match` 成功转成活动后，任务推进到 `activity_created → done`
- `cancel_match` 不结束整条任务，而是回到 `awaiting_match`，继续等待下一次更合适的匹配
- authenticated 用户触发 `create_activity` 后，任务推进到 `draft_collecting`
- 草稿真实写入 `activities(status='draft')` 后，任务推进到 `draft_ready`，并绑定真实 `activityId`
- `edit_draft / save_draft_settings` 不创建新的任务，而是在原任务上补充上下文并维持 `draft_collecting / draft_ready`
- `publish_draft / confirm_publish` 成功后，任务推进到 `published → done`
- `GET /ai/tasks/current` 已作为 runtime 快照查询接口供联调、回归与后续承接面接入；当前 H5 首页会直接拉取该接口，并根据 `homeState + primaryTaskId + items[]` 选择主状态卡与 primary task
- `GET /ai/tasks/current` 的任务快照可返回 `attentionLevel`：`time_sensitive` 用于活动快开始等时间敏感事项，`action_required` 用于待确认匹配等需要立即处理事项，`follow_up` 用于活动后反馈 / 再约等收尾事项，`normal` 或空值用于普通继续态
- `resolveCurrentTaskHomeState` 必须优先把 `match_ready`、`time_sensitive`、`action_required` 这类状态映射为 H3 待处理 / 待出发；`follow_up` 可映射为 H4 收尾态或消息中心行动项
- 当 `find_partner` 任务进入 `match_ready` 时，任务快照的推荐动作可附带 `matchId` 聚焦信息；消息中心或待确认匹配详情应直接承接到对应匹配，而不是让用户重新找一遍
- 消息中心、pending match 详情、讨论区与 post_activity follow-up 必须明确表现为同一条任务的后续承接面；当前 H5 首页已渲染主状态卡，消息中心仍以承接与恢复入口为主，不额外做“任务中心式胶囊列表”

**join_activity 跨端真相源约束（v5.5）**：
- `join_activity` 的领域真相由 Activities API 提供，客户端不得各自发明“报名成功”语义
- `POST /activities/:id/join` 必须返回结构化结果：`joined | already_joined | waitlisted | closed`
- `GET /activities/:id` 必须返回 `joinState + canJoin + isFull + remainingSeats`
- Mini Program、Web/H5、Admin 只能在表现层做差异化，不得在消费层各自重新推导 join 结果

**当前约束**：
- 不要求前端全量持有 `taskId`；允许后端通过 `userId + activityId + active join task` 解析当前任务
- 游客仍可浏览与探索，但数据库级任务 ownership 从 authenticated 用户开始
- `find_partner` 通过 `partnerIntentId / intentMatchId` 与真实异步结果关联；`create_activity` 通过真实 `activityId` 持续拥有草稿到发布链路

### 5.6 关键词运营模块（补充信号层）

`hot-keywords` 相关表、接口和 processor 在实现上仍然存在，但在新的产品主轴下，它的角色已经收口为：

- 内容运营与外部分发的关键词管理
- `/ai/chat` 入口前的补充信号
- 局部快捷入口或运营实验的配置源

它不再被定义为产品主流程的中心路由，也不再要求“命中关键词后直接替代主链思考”。
...
- 命中关键词时，可补充 metadata、运营上下文或少量快捷承接
- 未命中时，主流程完全不受影响
- 是否“直接返回预设响应”应视具体关键词配置与业务场景决定，不能再把它写成统一产品原则

**核心 API**：

```typescript
// POST /hot-keywords/match - 关键词运营匹配接口
...
```typescript
// 转化追踪逻辑
// 1. 用户触发关键词直达或运营入口 → 记录 hitCount
// 2. 用户完成转化 (报名/发布/入池等) → 查询最近 30 分钟内的关键词命中
// 3. 如果找到关联关键词 → 记录 conversionCount
...
| 功能 | 说明 |
|------|------|
| 关键词列表 | 支持搜索、筛选（状态、匹配方式、响应类型）、批量操作 |
| 关键词表单 | 创建/编辑关键词，支持 JSON 编辑器配置响应内容 |
| 关键词分析 | 命中率 Top 10 柱状图、转化率分析表格 |

### 5.5 SSE 事件

> 下表为早期 SSE 事件历史。当前 H5 / 小程序主链路统一使用 `GenUIStreamEvent`，并遵循“`text block` 先到，`structured block` 后到”的时序约束；客户端按事件顺序直接渲染，不再对正文做启发式重排。
...
这一章用一套更直白的中文来描述主链路：入口层、理解层、处理层、执行层、状态层、输出层、保障层。它描述的不是一个“聊天接口”，而是一条把用户输入持续推进到领域结果的主链路。
...
│  ┌──────────────────────▼────────────────────────┐              │
│  │  模型路由（Model Router）                        │              │
│  │  Moonshot / Kimi 主力，默认不启用额外回退        │              │
│  └──────────────────────┬────────────────────────┘              │
│                         │                                        │
│  ┌─────────────┐  ┌─────▼─────┐  ┌─────────────┐              │
│  │安全护栏      │  │提示词工程    │  │可观测性       │              │
│  │(Guardrails)  │  │(Prompts)     │  │(Observability)│              │
│  │输入/输出检测 │  │人设与提示词  │  │追踪/日志/指标 │              │
│  └─────────────┘  └───────────┘  └─────────────┘              │
│                                                                  │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐              │
│  │工作流        │  │评估系统       │  │内容审核       │              │
...
1. `ai.service.ts` 是编排总线（串联执行、trace、能力门面导出）。
...
5. 默认不保留长期兼容 API alias；如确有迁移需要，必须限时并最终删除。
...
├── ai.service.ts         # 顶层主门面（对话入口 + 子域能力导出）
│
├── runtime/              # 对话运行时内部实现
│   ├── chat-response.ts  # 统一对话入口编排（请求规范化 / response.blocks / SSE）
│   └── response-policy.ts # 响应策略层（fallback / 结构化块策略）
│
├── processors/           # Processor 管线架构 (纯函数)
...
│   ├── keyword-match.ts  # P0 关键词匹配
│   ├── intent-classify.ts # P1 意图分类（Feature Combination 单层）
...
│   └── genui-blocks.ts   # 界面块工厂与 dedupe 逻辑（入口层 / 策略层共用）
│
...
│   ├── classifier.ts     # 分类器入口
│   ├── feature-combination.ts # P1 Feature_Combination 规则引擎
...
│   ├── temporal-decay.ts # 时间衰减函数
│   ├── user-memories.ts  # 偏好记忆持久化
│   └── memory.service.ts # 记忆查询与管理服务
│
├── tools/                # 工具系统模块 (整合式架构)
...
│   ├── explore-nearby.ts # 探索附近（RAG 语义搜索）
│   ├── partner-match.ts  # 找搭子确认/取消
│   └── widget-protocol.ts # Widget 协议字段
│
├── models/               # 模型路由模块
│   ├── types.ts          # ModelConfig, ChatParams, ModelIntent
│   ├── router.ts         # 模型选择、回退与 Embedding/Rerank 入口
│   ├── runtime.ts        # runText/runObject/runStream
...
│   ├── interpolator.ts   # 模板变量插值
│   └── prompt-template.service.ts # System Prompt 读取与模板拼装
│
├── workflow/             # HITL 工作流模块
...
### 6.3 理解层：意图识别 (Intent Classification)
...
用户输入
  │
  ▼
P0: 关键词补充信号（keyword-match-processor）
  │ 命中 → 补充 metadata、可选快捷承接或局部直达
  │ 未命中 ↓
  ▼
P1: 规则组合识别（Feature Combination）
  │ 命中 → 返回分类结果
  │ 未命中 → 返回 unknown
```
...
- `P1 Feature Combination`：承担主意图识别
- `P0 keyword-match`：只提供补充信号，不再被视为首页或主流程的中心入口
- P1 未命中时返回 `unknown`，由后续主链路显式处理，不再升级到 LLM 分类
...
**Intent Classify Processor 设计原则**：
...
| **语义回忆** (Semantic Recall) | 向量检索相关活动 + 对话消息 | `activities.embedding` + `conversation_messages` | ✅ 已实现 |
...
interface EnhancedPreference {
  ...
  mentionCount: number;    // 提及次数（初始值 1）
...
- `followUpMode='review'` 的对话会在 completed 活动场景下把生成结果提炼后写入 `activityOutcomes.reviewSummary`
...
偏好提取由 `memory-summarize` 定时任务负责汇总写入 `user_memories`，不再在对话 Async 阶段通过 LLM 实时提取。

**对话历史 (Conversation History)**：

- 24 小时会话窗口：同一用户 24h 内的消息归入同一会话
- 自动保存：`/ai/chat` 的 SSE 主链在响应完成后统一写入 `conversation_messages`
- 活动关联：Tool 返回的 `activityId` 自动关联到消息

**Memory Context 加载**：

`ai.service.ts` 中 `loadUserMemoryContext` 直接从 `user_memories` 表读取用户最近 3 条记忆，按 `updatedAt` 倒序，格式化为文本注入 System Prompt：
...
#### 6.5.1 输出层：界面块协议 (GenUI Protocol)
...
export interface WidgetChunk {
  /** 界面块类型，对应 conversationMessageTypeEnum */
  messageType: string;
  /** Widget 数据 */
...
### 6.8 执行层：HITL 工作流 (Human-in-the-Loop)
...
const result = await runEval({
  ...
  return {
    output: extractAssistantText(envelope.response.blocks),
...
- **入口层**：`apps/api/src/modules/ai/ai.service.ts` 作为顶层主门面，内部由 `runtime/chat-response.ts` 构建 `ChatRequest`
- **理解层**：`ChatRequest.structuredAction` 直达 `handleStructuredAction`，避免按钮点击退化为纯文本
...
- **上下文组装在分支判定之前**：所有请求先经过完整的 Pre-LLM 管线（userProfile、intentClassify），Action 快速出口在上下文就绪后才判定，同样享受 memory 增强
...
**统一链路原则**：所有请求先经过完整的上下文加载和 Processor 管线，Action 不是独立分支，而是统一链路末端的一个**快速出口**。Pre-LLM 管线先执行，产出 `userProfile`、`intentClassify`；Action 预处理（`handleStructuredAction`）在 Pre-LLM 之后执行，直接消费 Pre-LLM 产物，不再内部补查数据库。

```
限流 → 输入护栏 → System Prompt 基础组装 → keywordMatchProcessor
    │
    ▼
Pre-LLM Pipeline（intent-classify → [user-profile] → token-limit）
    │
    ▼
...
- Action 请求同样享受了 userProfile、intentClassify 等上下文增强的好处
- Action handler 通过 `ActionHandlerContext` 消费 Pre-LLM 产物：`userProfile`（画像元数据），不再在 handler 内部补查数据库
...
| `intentClassifyProcessor` | Pre-LLM | P1 Feature Combination 单层意图分类，未命中返回 `unknown` |
| `userProfileProcessor` | Pre-LLM (并行) | 从 `user_memories` 读取最近记忆，注入 Prompt 并写入 metadata 供 Action handler 消费 |
...
| 阶段 | 编排器 | 策略 | 失败行为 |
|------|--------|------|---------|
| Pre-LLM | `runProcessors()` | 串行 + 条件并行 (`parallelGroup`) | `intentClassifyProcessor` 永不失败；其他失败记录日志但继续执行 |
| Post-LLM | `runPostLLMProcessors()` | 串行 | 失败记录日志，继续后续 |
| Async | `runAsyncProcessors()` | `Promise.allSettled` | 失败静默，不影响响应 |

**执行边界**：
- **Pre-LLM**：所有请求必经（包括 Action 请求），确保 memory、意图分类等上下文已就绪
- **Post-LLM / Async**：仅 LLM 分支执行。Action 快速出口在 Pre-LLM 后直接返回，不经过 Post-LLM 和 Async 阶段
- Action 请求同样享受 userProfile 的上下文增强，但由 `buildStructuredActionResult` 直接组装响应，不走 `streamText`
...
```typescript
// 固定顺序：intent-classify → [user-profile] → token-limit
...
**Pre-LLM —— `userProfileProcessor` + `loadUserMemoryContext`**：

`ai.service.ts` 在 `executeChatRequest` 中直接查询 `user_memories` 表，加载用户最近 3 条记忆注入 System Prompt。`userProfileProcessor` 从 `user_memories` 读取并格式化画像文本，供 Action handler 消费。

`exploreNearbyTool` 在 Tool 执行时直接调用 `rag.search()` 进行语义搜索，不再依赖 Pre-LLM 预热召回结果。
...
5. **Pre-LLM 管线**：`buildPreLLMPipeline` → `runProcessors`，产出 `userProfile`、`intentClassify`
6. **意图分类读取**：从 `metadata.intentClassify` 提取 `intent`
7. **找搭子状态机检查**：`recoverPartnerMatchingState` → 若处于追问态则直接返回
8. **Action 预处理**：若存在 `structuredAction`，执行 `handleStructuredAction`，传入 `actionHandlerContext`（含 Pre-LLM 产出的 `userProfile`）
9. **Action 快速出口**：若 `structuredAction && actionResult`，调用 `generateActionVoice`（Kimi 轻量调用，~50 字）→ `buildStructuredActionResult` → 直接返回
10. **Tools 解析**：`resolveToolsForIntent`
11. **Prompt 拼接 + Audit**：System Prompt + `userProfilePrompt`，并记录 `system_prompt_audit` 日志（totalLength / baseLength / enrichmentLength / enrichmentCount）
...
| 函数 | 文件 | 职责 |
|------|------|------|
| `buildPreLLMPipeline` | `processors/pipeline.ts` | 返回 ProcessorConfig 数组：intent-classify → [user-profile] → token-limit |
| `runProcessors` | `processors/index.ts` | 串行 + 条件并行编排 |
| `intentClassifyProcessor` | `processors/intent-classify.ts` | P1 Feature Combination 单层意图分类，未命中返回 `unknown` |
...
| `resolveToolsForIntent` | `tools/registry.ts` | 按 intent 动态加载 Tools 子集 |
...
| `summarizeUserMemories` | `jobs/memory-summarize.ts` | 定时任务汇总用户记忆写入 `user_memories` |
...
sequenceDiagram
    ...
    Note over LLM, Search: Phase 0: 即时搜索
    LLM->>Search: 提取搜索偏好 JSON (Type, Time, Area, Description)
    Search->>DB: 查询可推荐候选（active intents / 开放候选）
    Search->>Search: 语义召回 + 规则重排
    Search-->>User: 返回结果卡 / 匹配理由

    Note over LLM, API: Phase 1: 明确选择后入池
    LLM->>API: 用户点击“继续帮我留意”
    API->>API: 计算 Embedding / 结构化标签
...
- 相关约束补充见 **CP-57 ~ CP-61**（搜索先行、游客可搜索、显式入池、候选卡隐私保护、首轮表单必须是“找人表单”）。

---

### 6.15 执行层：极速组局 (Mapping Flow 1.2.2)
...
- 首屏引导包括：加入成功标题、活动协作说明、1 条集合 / 时间 / 地点确认提示
- H5 与小程序共享同一 `join_activity` 真相源；即使某些写动作仍在逐步补齐，也不得再把 H5 定义为“仅降级浏览页”
- 页面首次成功承接 `join_success` 时，需通过 Orval SDK 调用 `POST /ai/tasks/discussion-entered`
- 后端按 `userId + activityId` 解析当前 `join_activity` 任务并推进到 `discussion`
- 如果 `joinResult = waitlisted | closed`，客户端不得伪装成“报名成功进入讨论区”，而应停留在当前详情态并展示明确反馈
- `stay_on_detail` 属于 join 完成态的一种合法导航意图，用于跨端统一承接候补与关闭状态
...
apps/web 是面向用户的 H5 轻端产品，基于 Next.js 15 构建。它不是单纯的“降级页”或“邀请函站点”，而是与小程序共享同一产品哲学的承接面：`/chat` 是统一首页主路由，活动详情分享态承担外部分发与回流。
...
| SEO | 无 | 公开分享页可被搜索引擎索引 |
| 定位 | 内部管理工具 | 面向用户的轻端承接面 |

**关键约束**：
- Next.js 只做前端渲染层，**不使用 API Routes**
- 所有数据请求走 Elysia API（无 BFF）
- 对话入口统一调用 `POST /ai/chat`（SSE 界面块响应）
- `/chat` 是 H5 首页主路由，状态首页与对话主舞台共用同一页面
- **Mobile-First 响应式设计**：所有页面以移动端为基准设计，桌面端通过 `max-w-lg`（分享页）/ `max-w-[430px]`（对话页）居中约束内容宽度
...
| 主题轻量强调 | 内置函数 | 按活动类型解析卡片强调色，整页背景保持统一任务舞台 |
...
├── lib/
│   ├── eden.ts                    # Eden Treaty 客户端（预留，后续阶段）
│   ├── pending-agent-action.ts    # H5 写入动作被 auth gate 打断时的页面内恢复兜底
│   └── themes.ts                  # 预设主题配置（与 API 端同步）
├── next.config.ts
├── package.json                   # @xu/web
├── tailwind.config.ts
...
**活动详情视觉边界**：
- 活动详情默认收敛为黑色任务舞台，避免不同活动主题造成大面积视觉分叉
- 活动 `theme/themeConfig` 可作为活动卡片内的轻量强调色，不再驱动整页背景氛围
- 首页空态视觉是 H5 产品感的必要部分，React Bits 仅允许作为首页视觉包装层，不替代活动详情、报名、讨论承接逻辑

**报名与讨论承接**：
- 游客可以浏览公开详情与最近讨论摘要
- 报名、进入讨论区等写入动作统一触发 H5 登录/绑定手机号动作闸门
- 报名动作被 auth gate 打断时，H5 会写入 `pending-agent-action` 作为页面内恢复兜底；如果用户已有 token，则同步调用 `POST /ai/tasks/join-auth-gate` 写入服务端任务挂起点
- 报名成功后统一进入 `join_success -> discussion -> action prompt` 链路
...
- 通过 `fetch` + SSE 调用 `POST /ai/chat` 端点
- 通过 `GET /ai/tasks/current` 拉取当前 open task snapshots，并消费 `homeState + primaryTaskId + attentionLevel` 决定首页主状态卡与紧急程度表达
- 支持在同一 assistant 气泡内渲染文本 + 结构化 block
- 与小程序共享同一产品哲学，在小程序不可用时也能独立承接主流程
- 当前实现包含消息中心抽屉、侧边抽屉与状态首页承接，不再把 H5 定义为“无消息中心的游客降级页”
- `MessageCenterDrawer` 完成取消匹配、活动后轻反馈等写入动作后，会回调刷新 `/chat` 当前任务快照，避免“已处理但首页仍催我”的状态滞后
- `post_activity` 在 H5 消息中心首屏优先提供 `挺顺利 / 一般 / 没成局` 轻反馈；复盘或再约由 `POST /participants/self-feedback` 返回的 `nextAction` 承接
- H5 消息中心动作分流遵循最短路径：`self-feedback`、`confirm/cancel match` 这类确定写入动作直接调用 API；`discussion_reply`、`activity_reminder`、群聊摘要这类有明确对象页的动作直接跳活动详情 / 讨论承接；只有复盘、再约、草稿继续、开场白这类需要 agent 生成或理解的动作才通过 `/chat`
...
- 首页空态的视觉包装只能包在现有组件外层：
  ...
  - 允许用 React Bits 包装输入框边界或轻背景层，但轻背景层只能作为页面最底层 bg，必须低于 Hero、状态卡、任务卡和 CTA
  - 禁止自定义重写消息主列表、消息协议或输入框主逻辑
- 进入任务主舞台后禁止保留大面积欢迎背景，避免与消息和 GenUI card 抢占注意力
...
| 组件 | 用途 |
|------|------|
| `BorderGlow` (React Bits) | 首页空态输入框外层的轻边界呼吸包装 |
...
- `eden.ai.chat.post({ message })` — 对话入口（SSE）
...
### 7.0 GenUI vNext 请求与渲染链路（2026-03）

小程序端采用 `schema-first + local runtime`，不直接复用 monorepo 运行时代码：
...
- 页面状态由 `apps/miniprogram/src/stores/chat.ts` 维护：`text/action` 输入 + `context.lat/lng/activityId/followUpMode` -> `/ai/chat`（GenUI payload） -> `response.blocks` 渲染
- `choice` block 被映射为 `widget_ask_preference`，在同一条 assistant message 中同时渲染文本与交互卡片

说明：

- `/ai/chat` 为 H5 与小程序统一主链路，确保多端同构的 GenUI 实时渲染
- Action 输入在入口层映射为 `ChatRequest.structuredAction`，避免按钮点击退化为文本意图识别
...
- 返回主场后进入首页空态，以 Hero + 3 条建议入口 + Composer 承接，不再依赖 `Widget_Dashboard`
- **MVP**：使用默认问候语即可
...
### 9.1 Admin Cockpit Redesign

**设计理念**：从传统 CRUD 管理系统转型为 Solo Founder 的轻量 Ops Workbench。`找搭子 / 组局 Agent` 是主引擎，Admin 负责看盘、分发和人工兜底，不追求自建完整运营中台。
...
组局
  └── 活动与搭子 - 活动盘面 + 待处理搭子意向 + 待跟进匹配

风控
  └── 风险审核 - 违规内容队列（通过/删除/封号）

AI
  ├── Playground - AI 工作区（调试视图 + 配置视图）
  ├── 对话记录 - 会话列表（问题排查/质量观察）
  ├── 模型路由 - 作为 Playground 内配置视图进入
  └── 用量统计 - Token 消耗与成本观察
...
这套信息架构不按角色拆后台，也不通过隐藏路由制造第二套系统；瘦身主要通过稳定领域分组、明确页面命名和页内局部聚合来完成。
...
- Tool：工具名（中英文映射）、输入/输出 JSON 查看器、界面块类型
...
- 说明：当前主聊天链路仍由 `getModelByIntent()` 按意图路由；Agent 默认运行 `kimi-k2.5`
...
- Transport：Playground 统一对齐 `/ai/chat` 的 GenUI SSE 与 trace 调试协议
...
| LLM | `model` = 实际 modelId | 调试视图展示真实 modelId，不再默认暗示 Qwen 主链路 |
...
- Tool 调用详情（input 参数 + output 返回值 + 界面块类型）
...
- **CP-20**: 对话自动持久化 - 有 userId 时保存到 conversation_messages
...
**意图分类**：
- **CP-36**: 单层意图分类 - `intentClassifyProcessor` 仅使用 P1 Feature Combination，未命中返回 `unknown`，不再升级到 P2
- **CP-37**: Processor 容错 - `intentClassifyProcessor` 始终返回 `success: true`，不阻断 pipeline；其他 Pre-LLM processor 失败记录日志但继续执行
...
- **CP-45**: `join_activity` 任务阶段单调推进，除 `cancelled/expired` 外，不允许从后置阶段回退到更早阶段
- **CP-46**: 报名成功后，`agent_tasks.activityId` 必须与真实报名的 `activityId` 一致，不得只停留在文本或临时状态
- **CP-47**: 讨论区 `join_success` 首次承接后，必须存在一次 `discussion_entered` 事件，用于证明报名后的协作阶段已真正开始
- **CP-48**: 活动后带 `activityId + followUpMode` 的 AI 入口，若能解析到对应 join 任务，必须将其推进到 `post_activity` 或 `done`
- **CP-49**: `opt_in_partner_pool / createPartnerIntent` 成功写库后，`agent_tasks.partnerIntentId` 必须与真实写入的 `partner_intents.id` 一致
- **CP-50**: 真正创建 `intent_matches` 时，相关 `find_partner` 任务必须推进到 `match_ready`，不能只停留在“已提交意向”
- **CP-51**: `cancel_match` 不能把整个找搭子任务直接判定为完成；如意向仍有效，任务必须回到 `awaiting_match`
- **CP-52**: `confirm_match` 成功创建活动后，相关 `find_partner` 任务必须带上真实 `activityId` 并进入 `done`
- **CP-53**: `create_activity` 草稿真实写库后，相关任务必须带上真实 `activityId` 并推进到 `draft_ready`
- **CP-54**: `edit_draft / save_draft_settings` 只能更新当前 `create_activity` 任务，不得额外创建新的并行发局任务
- **CP-55**: `publish_draft / confirm_publish` 成功后，相关 `create_activity` 任务必须记录 `published`，并最终推进到 `done`
- **CP-56**: `GET /ai/tasks/current` 只能返回当前用户仍处于 open status 的任务，并按 `updatedAt` 倒序输出；同时必须返回 `homeState + primaryTaskId`，供首页和其他承接层在同一份真相源上选定主状态
- **CP-57**: `search_partners` 成功时不得默认创建 `partner_intents`；只有显式 `opt_in_partner_pool` 才允许写库
- **CP-58**: Visitor 触发 `search_partners` 时必须返回可浏览结果；触发 `connect_partner / request_partner_group_up / opt_in_partner_pool` 时必须返回 `authRequired + pendingAction`
- **CP-59**: `widget_partner_search_results` 返回的候选卡不得包含联系方式、wxOpenId、精确 GPS 坐标或其他直接可识别私密信息
- **CP-60**: 只有用户显式选择“继续帮我留意”时，系统才允许创建 `partner_intents` 并进入持续匹配
- **CP-61**: 找搭子首轮表单必须以“描述想找什么样的人”为目标，`textarea` 为主信息源；不得默认要求费用方式、具体约点或公开标签选择
- **CP-62**: H5 活动详情报名被 auth gate 打断时，若已有登录 token，必须调用 `POST /ai/tasks/join-auth-gate` 写入可恢复 `join_activity` 任务；本地 pending action 只能作为页面内兜底
- **CP-63**: `GET /ai/tasks/current` 返回的任务快照若包含 `attentionLevel`，H5 与小程序只能按该字段做优先级表现，不得按标题或文案猜测紧急程度
- **CP-64**: H5 `post_activity` 承接必须先写入真实轻反馈，再根据后端 `nextAction` 承接复盘或再约；不得把复盘 / 再约作为替代真实反馈的第一动作
...
- 状态承接入口只能由后端基于任务与上下文选择，前端不得自行推断任务优先级
- 首页空态的视觉中心是 **Hero + 留白 + 输入框**
...
- H5 聊天主轴必须继续使用 **AI SDK Elements**：
  ...
  - `Message`
  - `PromptInput`
- 禁止为首页空态或聊天态重写消息主列表组件
- React Bits 允许用于：
  - 首页空态背景气氛层
  - 首页空态输入框边界 wrapper（例如 `BorderGlow`）
- 禁止用 React Bits 取代：
  - 聊天 transcript
  - SSE 流式消息容器
  - 用户/助手消息气泡协议
...
- **第三层：效果**
  - 只允许极轻背景气氛和边界呼吸
  - 不允许高饱和霓虹、强渐变、过重玻璃态
  - 效果层永远低于内容层；若视觉效果与文字、任务卡或 CTA 发生重叠，优先移动/降噪/遮罩效果层，而不是让内容去适配特效

#### 人工验收边界

视觉层级不纳入自动截图回归。H5 首页视觉改动以人工设计评审为准，检查顺序固定为：

1. **任务可读性**：当前主状态、主 CTA、输入框是否比动效更先被看到
2. **背景归位**：orb / glow / motion 是否只在 bg 层，且没有贴顶、穿字、穿卡片、截断 header
3. **容器克制**：header 是否保持透明浮层，状态卡是否有足够实底或遮罩承载文字
4. **产品必要性**：视觉元素是否服务“状态首页接住用户”，而不是制造展示感

### 13.5 小程序对齐规则

- 小程序首页空态与 H5 保持相同信息架构：
  - Hero
  - 3 条建议入口
  - Composer
