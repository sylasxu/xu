...
#### 5.3.1 任务运行时（v5.4：join_activity + find_partner + create_activity）

`/ai/chat` 是统一入口，并为 `join_activity`、`find_partner` 与 `create_activity` 增加任务运行时持久化：
...
这一章按 README 同一套分层语言展开：入口层、理解层、处理层、执行层、状态层、输出层、保障层。它描述的不是一个“聊天接口”，而是一条把用户输入持续推进到领域结果的主链路。
...
用户输入
  │
  ▼
P0: keyword-match-processor（补充信号 / 关键词运营）
